  ...
  return (
    <motion.div key="email-capture" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex w-full max-w-md flex-col items-center gap-8 pt-16">
      <div className="text-center">
        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-[#0F2A33]/10">
          <Mail size={28} className="text-[#0F2A33]" />
        </div>
      ...
      <form onSubmit={handleSubmit} className="flex w-full flex-col gap-4">
        ...
        <Button type="submit" size="lg" disabled={!isValid || isLoading} className="bg-[#0F2A33] text-white hover:bg-[#1a3d4a]">
